...
class exception_error:
    ...
    def exception_message(self) -> None:
        ...
        print_info ('Something went wrong.',type='red',uplinebreak=True)
        print_info (self.first,type='red',downlinebreak=True)
        ...
        print_info (self.third,type='red',downlinebreak=True)        
...
class execute:
    ...
    def __init__(self, command, parameter, value) -> None:
        try:
            ...
            print_info ('Exec interrupted by user.')
            return
    ...
    def execute_command(self) -> None:
        try:
            ...
            if check_result:
                print_info (f'Exec: {self.command} {self.parameter} {self.value}')
                os.system(self.cmd)
            else:
                raise TerasploitUnknownCommand(self.command,'Unknown command was specified in command prompt.')
...
class TerasploitExceptionMessage:
    ...
    def unknown_command_error(self):
        print_info (f"Unknown command error: '{f.GREEN}{self.cause}{f.RESET}', use '{f.GREEN}help{f.RESET}' to show all commands information.",type='RED')
        
    
    def module_required_error(self):
        print_info (f"Module required error: '{f.GREEN}{self.cause}{f.RESET}', no current module in use.",type='RED')
        

    def payload_required_error(self):
        print_info (f"Payload required error: '{f.GREEN}{self.cause}{f.RESET}', no current payload in use.",type='RED')


    def encoder_required_error(self):
        print_info (f"Encoder required error: '{f.GREEN}{self.cause}{f.RESET}', no current encoder in use.",type='RED')
        
    
    def invalid_option_error(self):
        print_info (f"Invalid option error: '{f.GREEN}{self.cause}{f.RESET}', use command '{f.GREEN}show options{f.RESET}' to display valid options.", type='RED')

        
    def module_exception_error(self):
        print_info (f"Module exception error: '{f.GREEN}{self.cause}{f.RESET}', module ended with an exception.", type='RED')
        
    
    def start_session_error(self):
        print_info (f"Start session error: '{f.GREEN}{self.cause}{f.RESET}', non-exploit module has attempted to start a session.", type='RED')
        
        
    def module_error(self):
        print_info (f"Module error: '{f.GREEN}{self.cause}{f.RESET}', failed to import! invalid module.",type='RED')
        
        
    def value_error(self):
        print_info (f"Value error: '{f.GREEN}{self.cause}{f.RESET}', no value was specified.", type="RED")
       
        
    def param_error(self):
        print_info (f"Parameter error: '{f.GREEN}{self.cause}{f.RESET}', no parameter was specified.", type="RED")
        
    
    def terminate_session(self):
        print_info (f"session terminated {time.strftime('%Z %H:%M:%S - %A, %B %e, %Y')}")
...
class TerasploitTerminateSession(TerasploitException):
    """ Raised when terminating a session """
    
    def __init__(self, exception_cause: str, exception_message: str):
        super().__init__(exception_cause,"terminate_session",exception_message)