...
from libs.terasploit.framework.command.util.decorator import param_required
from libs.terasploit.framework.opts.opt_distributor import Opt

from init.tsf.ui.wildcard import Table, info_print, line_break
...
class _show_:
    ...
    @param_required
    def function() -> None:
        query, opts = Get.parameter(), Get.options()
        ...
        if hasattr(_show_,f"{query.lower()}"):
            ...
            if len(inspect.getfullargspec(sub_function)) == 1:
                sub_function(opts)
            else:
                sub_function()
        else:
            if query in directories.get():
            ...
            if query not in directories.get():
