...
class TerasploitModule(Auxiliary):
    ...
    def initialize(self,info_only: bool = False) -> None:
        ...
        register_option ('auxiliary',opt=[
            ...
            OptValidate.create('port_range','port_range',['80-443','no','port range to scan, remove value to scan all port']),
            ...
            OptValidate.create('timeout','timeout',[0.5,'yes','socket connection timeout']),
            OptValidate.create('protocol','protocol',['tcp','yes','socket protocol to use'])
        ])
    ...
    def run(self) -> None: 
        ...
        print_info (f'Open Ports: {len(self.successful_scan)}')
        for i in self.successful_scan:
            print (f" -  {i}")
        print_info (f'Closed/Filtered Ports: {len(self.failed_scan)}')
