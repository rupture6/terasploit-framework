...
class TerasploitModule(Auxiliary):
    ...
    def initialize(self,info_only: bool = False) -> None:
        update_info (
            {
                ...
                'Description' : [
                    ...
                    '',
                    'Warning',
                    '   This module does not reset the http session, other modules',
                    '   may get affected as it will reused the session from this',
                    '   module.'
                ]
            }
        )
        ...
        register_option ('auxiliary',opt=[
            OptString.new('config','yaml',['','yes','yaml config to read']),
    ...
    def run(self) -> None:
        file = self.GetOPT('config')
        session = self.GetOPT('session')
        try:
