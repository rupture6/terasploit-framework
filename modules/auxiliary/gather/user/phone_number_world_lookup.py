...
class TerasploitModule(Auxiliary):
    ...
    def initialize(self,info_only: bool = False) -> None:
        ...
        register_option ('auxiliary', opt=[
            OptValidate.create('number','phone_number',['','yes','target phonenumber to lookup, do not put "+(cc)"'])
        ])
    ...
    def run(self) -> tuple[str,bool]:
        ...
        for codes in iso_codes:
            try:
                ...
                if phone_validity:
                    print_info (f'Phone number found in {f.CYAN}{codes}{f.RESET}',type='GREEN')
