...
class TerasploitModule(Auxiliary):
    ...
    def initialize(self,info_only: bool = False) -> None:
        ...
        register_option ('auxiliary',opt=[
            OptValidate.create('rhost','host',['','yes','the remote host target (url/ip)']),
            OptPort.create('rport',[80,'yes','the target remote port']),
            OptInt.create('byte',[4096,'yes','bytes to receive from socket']),
            OptValidate.create('type','socket_type',['SOCK_STREAM','yes','socket type to specify']),
            OptValidate.create('family','address_family',['AF_INET','yes','address family to specify']),
            OptValidate.create('http_type','http_type',['HTTP/1.1','yes','type of http to use']),
            OptValidate.create('method','http_method',['HEAD','yes','http method to use']),
            OptValidate.create('timeout','timeout',[60,'yes','socket connection timeout'])  
        ])
    ...
    def run(self) -> None:
        ...
        try:
            ...
            print_info (error,type='red',downlinebreak=True,uplinebreak=True)
            return